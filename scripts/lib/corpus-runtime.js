...
function bundleReadme() {
  return `# Temporary structured corpus staging\n\nThis directory is generated internally while publishing or restoring an immutable cloud corpus release. It contains only verified MySQL and Qdrant artifacts, is not a user-facing distribution format, and is removed after the operation.\n`;
}
  ...
  try {
    ...
    const manifest = {
      ...
      compatibilityNotes: [
        ...
        'Original uploads are excluded from structured staging and are collected separately into the same cloud release.',
    ...
    await verifyBundle(temporary, { quiet: true });
    ...
    if (!options.quiet) {
      console.log(JSON.stringify({
        ...
        totalBytes,
        artifactBytes: manifest.bundlePayloadBytes
      }));
    }
    return manifest;
  } finally {
...
async function verifyBundle(directory = BUNDLE_DIRECTORY, options = {}) {
  const manifestPath = path.join(directory, 'manifest.json');
  let manifest;
  try {
    manifest = JSON.parse(await fsp.readFile(manifestPath, 'utf8'));
  } catch (_error) {
    throw corpusError('CORPUS_BUNDLE_MISSING', 'A valid temporary structured corpus staging manifest is required.');
  }
