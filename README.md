...
- Original files dùng local shared volume. Portable corpus chỉ chứa sanitized MySQL + Qdrant snapshot.
...
Mở root `.env` và thay các giá trị demo cần thiết. Remote stack cần tối thiểu `GOOGLE_API_KEY`, `LLAMA_CLOUD_API_KEY`, `RAG_INTERNAL_TOKEN`, database password và auth secrets. Không commit `.env` hoặc đưa credential vào log/tài liệu.
