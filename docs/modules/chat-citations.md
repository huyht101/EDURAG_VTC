...
Citation source phải map `vectorNodeId` tới chunk `READY + VISIBLE` tại thời điểm trả lời. Page là 1-based khi có; với TXT/DOCX nó có thể là synthetic segment thay vì trang vật lý. Snapshot fragment/title/page/section/locator/scores vẫn đọc được sau khi document hide/delete. Citation API chỉ cho session owner, kể cả khi user có role ADMIN. Original file chỉ stream sau ownership check và current source authorization; `storage_key` không public. Cloud corpus restore đưa exact-approved original về local upload volume; GCS detail không public. Nếu fresh setup không restore corpus thì original unavailable, còn citation snapshot chỉ dùng được khi structured local state đã tồn tại.
