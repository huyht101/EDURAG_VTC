...
| Private corpus release/equivalence | Tooling, exact local marker and selected pointer exist | Qdrant snapshot is one store in bundle | N/A | N/A | Release `v1-d07f526e059e53751402a4f3` remote read-back, clean restore, lifecycle/cross-store consistency and local reset PASS | **LIVE RELEASE ACCEPTED** | Corpus operator: retain immutable prior releases and rerun exact verification after any future pointer change |
| Release-ID history | Pointer is `v1-d07f526e059e53751402a4f3`; it supersedes the selected legacy lifecycle-incompatible release without overwriting it | N/A | N/A | N/A | New release has a distinct source fingerprint after exact-attempt Qdrant payload repair; old release remains immutable | **VERIFIED SUCCESSION** | Corpus operator: preserve create-only/pointer-last workflow |
