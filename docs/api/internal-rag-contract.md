...
Trạng thái hiện tại: NodeJS adapter và contract tests đã triển khai; Python capability bên dưới là quan sát từ snapshot; remote end-to-end chưa chạy.
...
- Current snapshot callback sender đã gửi Bearer. Inbound routes chưa verify Bearer: **Required Python change**.
...
| `attempt_count` | target-required | Processing attempt; current snapshot schema chưa khai báo |
...
Current snapshot trả `202` với `status`, `job_id`, `message`. NodeJS từ chối response có `job_id` không khớp.

Target contract yêu cầu Python nhận và bảo toàn `attempt_count` từ ingest request để dùng cho mọi callback của processing attempt đó.
...
Current snapshot khai báo `job_id`, `action`, `callback_url`; `attempt_count` vẫn là target-required field cần Python preserve.
...
Current snapshot khai báo `job_id`, `callback_url`; `attempt_count` vẫn là target-required field.
...
Current snapshot khai báo `question`, `conversation_id`, `history`; Pydantic bỏ qua `request_id` và `user_id`. Đây là correlation extension phía NodeJS, không cấp authorization cho Python.
...
Current snapshot citation có `doc_id`, `page_number`, `snippet`, optional `chapter`, `section`. Target contract bắt buộc bổ sung UUID `vector_node_id`. NodeJS nhận `snippet` làm source fragment alias nhưng không suy đoán vector ID và không parse marker `[1]`.
